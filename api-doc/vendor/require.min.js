...
c=(1===d.length?["require"]:["require","exports","module"]).concat(c)));T&&(g=Q||pa())&&(b||(b=g.getAttribute("data-requiremodule")),e=K[g.getAttribute("data-requirecontext")]);e?(e.defQueue.push([b,c,d]),e.defQueueMap[b]=!0):W.push([b,c,d])};define.amd={jQuery:!0};k.exec=function(b){return eval(b)};k(v)}}
)(this);
/*左侧导航展开*/
$(document).on('click','#sidenav li a',function() {
    if (typeof($(this).attr('class')) == 'undefined') {
        var $group = $(this).parent().data('group')
        $(this).parent().parent().find('.nav-fold').slideUp();
        if ($(this).parent().nextAll('li[data-group="'+ $group +'"]').is(':visible')) {
            $(this).parent().nextAll('li[data-group="'+ $group +'"]').slideUp()
        } else {
            $(this).parent().nextAll('li[data-group="'+ $group +'"]').slideDown()
        }
    }
})
